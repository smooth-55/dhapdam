if(typeof jQuery!=='undefined'){(function($,undefined){function pad(str,ch,length){return(new Array(length+1-str.length).join(ch))+str}
...
if(h>24){if(str.length>=6){return $.fn.timepicker.parseTime(str.substr(0,5))}else{return $.fn.timepicker.parseTime(str+'0'+(am?'a':'')+(pm?'p':''))}}else{time.setHours(h,m,s);return time}}})()})(jQuery)}